...
local spawn = false
local money = false
local translation.blip = "Armored vehicle"
local translation.spotted = "~g~An armored vehicle has been spotted."
local translation.armored = "~r~You destroyed the armored vehicle!"
local translation.money = "Bag of money"
...
function fourgon_blip(entity)
	...
	AddTextComponentString(translation.blip)
...
Citizen.CreateThread(function()
	while true do
		...
		if spawn then
			...
			car = CreateVehicle(0x6827CF72, pos.x,pos.y,pos.z,pos.h, true, false)
		...
		if GetVehicleDoorAngleRatio(car,2) >= 0.0100 and GetVehicleDoorAngleRatio(car,3) >= 0.0100 then
			...
			if not money then
				...
				BeginTextCommandSetBlipName("STRING")
				AddTextComponentString(translation.bagofmoney)
		...
		if IsEntityAtCoord(GetPlayerPed(-1),obj.x, obj.y, obj.z,0.50,0.50, 4.0, 0, 1, 0) == 1 and IsPedOnFoot(GetPlayerPed(-1)) then
			TriggerServerEvent('vrp-fourgon:give')
			DeleteObject(bag)
			despawn()
		end
		...
		if DoesEntityExist(car) then
			if GetEntityHealth(car) == 0 then
				vRP.notify({translation.armored})
				removeblip(fourgon)
				despawn()
			end
		end
